...
type RecursiveExprTests() =
  ...
  let expr =
    recursive (fun expr ->
      ...
      Parse "expr" { return! bestOf [ binary "+"; binary "-"; nested; parseX ] })
  ...
  let ``can parse simple expr with parents`` () =
    ...
    match Parser.parse expr str with
    | Success x -> Assert.Equal(str.Length, x.View.End)
    | r ->
      System.Console.WriteLine "ohh No"
      System.Console.WriteLine(string r)
      Assert.True(false)
...
type RepetitiveExprTests() =
  ...
  let nested (expr : Parser<Expr, Unit>) : Parser<Expr, Unit> =
    Parse("nested", expr) {
      do! skipString "("
      let! x = expr
      do! skipString ")"
  ...
  let binary (expr : Parser<Expr, Unit>) : Parser<Expr, Unit> =
    Parse("binary", expr) {
      let! x = bestOf [ parseX; nested expr ]

      let! ys =
        (Parse() {
          let! op = bestOf [ parseString "+"; parseString "*" ]

          let! rhs = bestOf [ parseX; nested expr ]
      ...
      let exprTree =
        ys
        |> Seq.fold (fun tree branch -> tree |> extend branch) root
        |> seal
