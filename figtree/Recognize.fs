...
// not used here
let isRecursive (rt : RuleTable) (rule : Rule) =
...
let nonZeroRefs (rt : RuleTable) (rule : Rule) =
  let rec isNonEmpty (rule : Rule) =
    ...
    | EachOf xs ->
      ...
      |> Seq.isEmpty
      |> not
    | AnyOf xs -> xs |> Seq.forall isNonEmpty
    | Ref x -> isNonEmpty (RuleTable.rules rt).[x]
    | _ -> true
...
let recognize (rt : RuleTable) (current : string) (str : string) (i : int) =
  let rec applyRule (path : List<string * int>) rule j =
    match str.Length > j, rule with
    | false, _ -> None
    | _, CharSet xs ->
      if Set.contains str.[j] xs then
        Some <| Leaf(path, j, j + 1)
      else
        None
    | _, CharRange(a, b) ->
      if a <= str.[j] && str.[j] <= b then
        Some <| Leaf(path, j, j + 1)
      else
        None
    | _, Repeat(rule, a, b) ->
  ...
  applyRule ([ current, -1 ]) (RuleTable.rules rt).[current] i
