...
let right (bp : int) (op : 'Expr * 'Expr -> 'Expr) (rhs : 'Expr) : TreeExtension<'Expr> = (bp, (op, rhs))

let private extendUp
  (x : TreeAction<'Expr> -> 'Expr)
  ((op, rhs) : ('Expr * 'Expr -> 'Expr) * 'Expr)
  (action : TreeAction<'Expr>)
  =
  match action with
  | Down extend -> x (Top(fun lhs -> op (lhs, extend rhs)))
  | Top extend -> extend (x (Top(fun lhs -> op (lhs, rhs))))

let private extendDown
  (x : TreeAction<'Expr> -> 'Expr)
  ((op, rhs) : ('Expr * 'Expr -> 'Expr) * 'Expr)
  (action : TreeAction<'Expr>)
  =
...
let extend (b, right) (a, left) : OpenTree<'Expr> =
  if a <= b then
    b, (extendDown left right)
  else
    b, (extendUp left right)
